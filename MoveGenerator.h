...
namespace MoveGenerator {
    ...
    std::vector<sf::Vector2i> knightMoves(const sf::Vector2i& from, GameState state);

    std::vector<sf::Vector2i> QueenMoves(const sf::Vector2i& from, GameState state);

    std::vector<sf::Vector2i> PawnMoves(const sf::Vector2i& from, bool hasMoved, const sf::Vector2i& enpassantTarget, GameState state);
    std::vector<sf::Vector2i> PawnAttackMoves(const sf::Vector2i& from);

    std::vector<sf::Vector2i> KingMoves(const sf::Vector2i& from, GameState state);
